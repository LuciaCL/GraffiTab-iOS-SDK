...
@implementation GTGetNotificationsTask

- (void)getNotificationsWithStart:(int)start numberOfItems:(int)count successBlock:(void (^)(GTResponseObject *))successBlock cacheBlock:(void (^)(GTResponseObject *))cacheBlock failureBlock:(void (^)(GTResponseObject *))failureBlock {
    self.sBlock = successBlock;
    self.fBlock = failureBlock;
    self.cBlock = cacheBlock;
    
    NSString *string = [GTRequestBuilder buildGetNotifications];
...
- (id)parseJsonSuccessObject:(NSDictionary *)json {
    ...
    for (NSDictionary *itemJson in jsonArray) {
        GTNotification *n = [GTNotificationFactory createNotificationFromJson:itemJson];
