...
@implementation GTGetConversationsTask

- (void)getConversationsWithStart:(int)start numberOfItems:(int)count successBlock:(void (^)(GTResponseObject *))successBlock cacheBlock:(void (^)(GTResponseObject *))cacheBlock failureBlock:(void (^)(GTResponseObject *))failureBlock {
    self.sBlock = successBlock;
    self.fBlock = failureBlock;
    self.cBlock = cacheBlock;
    
    NSString *string = [GTRequestBuilder buildGetConversations];
...
- (id)parseJsonSuccessObject:(NSDictionary *)json {
    ...
    for (NSDictionary *personJson in jsonArray) {
        GTConversation *p = [[GTConversation alloc] initFromJson:personJson];
        [items addObject:p];
    }
